...
export default function TireNew() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await insertTires([newTire]);
    if (error) {
      setSaving(false);
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4">
        ...
        <div className="pt-2">
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <Save className="size-4" /> {saving ? "Saving…" : "Save tire"}
          </button>
        </div>
      </form>
    </div>
